...
- [x] Menutup security/operations launch gate: monitoring-alerting, vulnerability/dependency review, load test dasar, dan incident/support runbook.
  ...
  - [x] Pentest authenticated multi-tenant BOLA/IDOR dengan akun attacker/victim terpisah untuk Tenant Admin, Member, Guest, external meeting, dan Platform Admin lulus 1 September 2026. Gate ini juga menemukan dan menutup kegagalan audit actor lintas tenant melalui migrasi `0044`; laporan tersedia di `docs/pentest-authenticated-2026-09-01.md`.
  - [ ] Memperoleh pentest profesional independen sebelum enterprise general availability.
  - [x] Menghapus CSP production `unsafe-inline` memakai nonce per request yang kompatibel dengan Next.js, menghapus style attribute dari HTML browser, serta mengaktifkan `no-new-privileges`, drop seluruh capability, dan read-only root filesystem untuk API/web. Meeting integration dan recording MP4/signed-download/delete smoke lulus 2 September 2026; bukti tersedia di `docs/runtime-hardening-2026-09-02.md`.
