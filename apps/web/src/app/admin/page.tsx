...
type Dashboard = {
  ...
  health: { status: string; api: string; postgres: string; release: string; errors24Hours: number; databaseOpenConnections: number; checkedAt: string };
  observability: { database:{openConnections:number;inUse:number;idle:number;maxOpenConnections:number;waitCount:number;waitDurationMs:number}; media:{activeRooms:number;waitingParticipants:number;joinedParticipants:number;failedRecordings:number;driveFiles:number;rooms:number;upcomingEvents:number;chatMessages24Hours:number;driveStorageBytes:number;chatStorageBytes:number;totalStorageBytes:number} };
};
...
export default function AdminDashboard() {
  ...
  return <main className={styles.page}>
    <header><div><p>XSPACE ADMIN</p><h1>{dashboard.tenant.name}</h1><span>Workspace operations and usage overview</span></div><nav><Link href="/admin/meetings">Meetings</Link><Link href="/admin/users">Users</Link><Link href="/admin/groups">Groups</Link><Link href="/admin/roles">Roles</Link><Link href="/admin/scim">SCIM</Link><Link href="/admin/governance">Governance</Link><Link href="/admin/incidents">Incidents</Link><Link href="/admin/billing">Plan & usage</Link><Link href="/admin/audit">Audit</Link><Link href="/admin/policies">Policies</Link><Link href="/admin/settings">Settings</Link><Link href="/">← Workspace</Link></nav></header>
    <section className={styles.health}><span><i/> All monitored services operational</span><div><b>Release</b> {dashboard.health.release}</div><div><b>Errors 24h</b> {dashboard.health.errors24Hours}</div><div><b>API</b> {dashboard.health.api}</div><div><b>PostgreSQL</b> {dashboard.health.postgres}</div><div><b>DB connections</b> {dashboard.health.databaseOpenConnections}</div></section>
    ...
    <section className={styles.grid}>
      <article className={styles.panel}><div className={styles.panelHead}><div><h2>Database pool</h2><p>Live application connection telemetry</p></div><strong>{dashboard.observability.database.inUse} in use</strong></div><dl className={styles.usage}><div><dt>Open / idle</dt><dd>{dashboard.observability.database.openConnections} / {dashboard.observability.database.idle}</dd></div><div><dt>Pool capacity</dt><dd>{dashboard.observability.database.maxOpenConnections||"Unlimited"}</dd></div><div><dt>Wait count</dt><dd>{dashboard.observability.database.waitCount.toLocaleString()}</dd></div><div><dt>Wait duration</dt><dd>{dashboard.observability.database.waitDurationMs.toLocaleString()} ms</dd></div></dl></article>
      <article className={styles.panel}><div className={styles.panelHead}><div><h2>Media & recording</h2><p>Live meeting pipeline health</p></div><strong>{dashboard.observability.media.failedRecordings?`${dashboard.observability.media.failedRecordings} failed`:"Healthy"}</strong></div><dl className={styles.usage}><div><dt>Active rooms</dt><dd>{dashboard.observability.media.activeRooms}</dd></div><div><dt>Joined / waiting</dt><dd>{dashboard.observability.media.joinedParticipants} / {dashboard.observability.media.waitingParticipants}</dd></div><div><dt>Failed recordings</dt><dd>{dashboard.observability.media.failedRecordings}</dd></div><div><dt>Client errors 24h</dt><dd>{dashboard.health.errors24Hours}</dd></div></dl></article>
      <article className={styles.panel}><div className={styles.panelHead}><div><h2>Collaboration & storage</h2><p>Tenant-scoped resource telemetry</p></div><strong>{formatBytes(dashboard.observability.media.totalStorageBytes)}</strong></div><dl className={styles.usage}><div><dt>Drive files / storage</dt><dd>{dashboard.observability.media.driveFiles} / {formatBytes(dashboard.observability.media.driveStorageBytes)}</dd></div><div><dt>Chat messages 24h</dt><dd>{dashboard.observability.media.chatMessages24Hours}</dd></div><div><dt>Chat attachments</dt><dd>{formatBytes(dashboard.observability.media.chatStorageBytes)}</dd></div><div><dt>Rooms / upcoming events</dt><dd>{dashboard.observability.media.rooms} / {dashboard.observability.media.upcomingEvents}</dd></div></dl></article>
    </section>
  </main>;
}
